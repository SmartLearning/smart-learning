...
@Component({
    selector: 'smart-password-strength-bar',
    template: `
        <div id="strength">
...
export class PasswordStrengthBarComponent {
    ...
    measureStrength(p: string): number {
        ...
        force = (p.length <= 6) ? Math.min(
            force,
            10
        ) : force;

        // penality (poor variety of characters)
        force = (passedMatches === 1) ? Math.min(
            force,
            10
        ) : force;
        force = (passedMatches === 2) ? Math.min(
            force,
            20
        ) : force;
        force = (passedMatches === 3) ? Math.min(
            force,
            40
        ) : force;
